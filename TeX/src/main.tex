...
\begin{document}
    ...
    \begin{definition}[Произведение тензора и матрицы по измерению]
        Пусть $\mathcal A$ "--- тензор размерности $I_1\times I_2\times\ldots\times I_N$, $\mathbf U$
        ...
        \times J_n\times I_{n+1}\times \ldots\times I_N$, который считается по формуле
        \[
            (\mathcal{A}\times_n \mathbf U)_{i_1 i_2\ldots i_{n-1}j_n i_{n+1}\ldots i_N} = \sum_{i_n=1}^{I_n} a_{i_1 i_2\ldots
            i_{n-1}i_n i_{n+1} \ldots i_N} u_{j_n i_n}.
        \]
    \end{definition}

    \begin{theorem}
        [Сингулярное разложение порядка $N$]
        Любой комплекснозначный тензор $\mathcal{A}$ размерности $I_1\times I_2 \times \ldots \times I_N$ может быть представлен
        в виде произведения
        \begin{equation}
            \mathcal{A} = \mathcal{Z} \times_1 \mathbf{U}^{(1)} \times_2 \mathbf{U}^{(2)} \times_3 \ldots \times_N \mathbf{U}^{(N)},\label{eq:hosvd}
        \end{equation}
        в котором
        \begin{enumerate}
            \item $\mathbf{U}^{(n)}=\left[U_1^{(n)},\, U_2^{(n)},\ldots,\, U_{I_n}^{(n)} \right]$ "--- унитарная матрица,
            \item $\mathcal{Z}$ "--- комплекснозначный тензор размерности $I_1\times I_2 \times \ldots \times I_N$, в котором
    ...
    \begin{definition}
        \label{def:hosvd}
        Разложение вида~\eqref{eq:hosvd} будем называть сингулярным разложением тензора $\mathcal{A}$ порядка $N$ или
    ...
    \begin{remark}
        ...
        \begin{equation}
            \mathcal{X}=\sum_{i_1=1}^{I_1} \sum_{i_2=1}^{I_2}\ldots \sum_{i_N=1}^{I_N} \mathcal{Z}_{i_1,i_2,\ldots,i_N}
            U^{(1)}_{i_1} \circ U^{(2)}_{i_2} \circ \ldots\circ U^{(N)}_{i_N}.\label{eq:sum-hosvd}
        \end{equation}
    ...
    Пусть дан временной ряд $F$ длины $N$
    \[
        F=(f_1,f_2,\ldots,f_N).
    \]
    \begin{definition}(Траекторный тензор ряда)
        Траекторным тензором ряда $F$ с параметрами $I,L: 1\leqslant I,L \leqslant N,\, I + L \leqslant N + 1$
        ...
        \]
    \end{definition}
    Слои траекторного тензора ряда $F$ с параметрами $I, L$ имеют следующий вид
    \[
        \mathcal{X}_{,,j}=
        \begin{pmatrix}
    ...
    Алгоритм Tensor SSA заключается в проведении следующих четырёх шагов.
    \begin{enumerate}
...
%            \label{eq:tens-group}
        \end{equation*}
        \item Восстановление рядов $F^{(k)}=F^{(\mathfrak{S}_k)}$ по тензорам $\mathcal{X}^{(\mathfrak{S}_k)}$ посредством их усреднения вдоль
        ...
        \end{gather*}
    \end{enumerate}
    Результатом алгоритма является набор временных рядов $F^{(1)},\ldots,\, F^{(m)}$ такой, что
    \[
        F=\sum_{k=1}^{m}F^{(k)}.
    \]
    ...
    CANDECOMP-PARAFAC~\cite{parafac1, parafac2} "--- итерационный метод рангового разложения тензора, который по параметру $K$ считает наилучшее приближение
