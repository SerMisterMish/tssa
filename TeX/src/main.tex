...
%        $\widetilde{\Lambda}^{(K)}$.
    ...
    \begin{itemize}
        ...
        \begin{remark}
            ...
            Например, при $N=59$ и $L=30$ эти ряды будут слабо разделимы в терминах HOSVD-MSSA.
        \end{remark}
...
%    \FloatBarrier
    ...
    которых метод HOSVD-MSSA разделяет компоненты точнее, чем метод MSSA, увеличивается, однако
