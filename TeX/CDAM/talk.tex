...
\begin{frame}{ESPRIT Frequency Estimation Example}
  ...
  \begin{tikzpicture}[remember picture, overlay]
    ...
    {
      \includegraphics[width=0.52\textwidth]{Earth_complex.pdf}
    };
  \end{tikzpicture}
...
\begin{frame}{Some tensor decompositions}
  \begin{itemize}
    \item \bluetext{$(L_r, L_r, 1)$}: sum of tensors with elements of
      ...
      $D^{(r)}$ and matrix $B^{(r)}$ of rank $L_r$
      \begin{itemize}
        \item Considered for signal extraction and decomposition in
          [De~Lathauwer~(2011)]
    ...
    \item \bluetext{HOSVD}: $\calX = \sum_{i=1}^{I}\,
      \sum_{l=1}^{L}\, \sum_{k=1}^{K}\, \calZ_{ilk} U_i^{(1)} \circ
      U_l^{(2)} \circ U_k^{(3)}$
      \begin{itemize}
...
\begin{frame}{Dtack Modifications}
  $\tX = (x_1,\, x_2,\, \ldots,\, x_N)$, $M = \lfloor N / D\rfloor$
  \[
    ...
    \left[
      \begin{NiceArray}{c|c|c|c}
        ...
        &\ldots & x_{MD} \\
        % \multicolumn{1}{c}{\underbrace{\hphantom{x_{(M -
        % 1)D+1}}}_{\tX_D^{(1)}}}
        \CodeAfter
