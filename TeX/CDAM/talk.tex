...
\begin{frame}{Decomposition and Estimation Example}
  Data: Coordinates of Earth pole motion [IERS EOP 14 C04]\\
  Plotted each over time and against each other
  ...
  \begin{tikzpicture}[remember picture, overlay]
    \node[right=3.3cm, below=2.2cm] at (current page.north west)
    {
      \includegraphics[width=0.49\textwidth]{Earth_x.pdf}
    };
  \end{tikzpicture}

  \begin{tikzpicture}[remember picture, overlay]
    \node[left=3.3cm, below=2.2cm] at (current page.north east)
    {
      \includegraphics[width=0.49\textwidth]{Earth_y.pdf}
    };
  \end{tikzpicture}
  ...
  \begin{tikzpicture}[remember picture, overlay]
    ...
    {
      \includegraphics[width=0.58\textwidth]{Earth_complex.pdf}
    };
  \end{tikzpicture}
...
\begin{frame}{Decomposition and Estimation Example}
  ...
  \begin{columns}
    \begin{column}{0.5\textwidth}
      \begin{figure}[!ht]
        \center
        \includegraphics[width=0.99\textwidth]{Earth_complex_signal.pdf}\\
        Extracted signal
      \end{figure}
    \end{column}
...
\begin{frame}{Introduction of Tensors}
  \begin{table}
    \hspace*{-0.9cm}
...
\begin{frame}{Some tensor decompositions}
  \begin{itemize}
    \item \bluetext{CPD}: sum of rank-1
      tensors ($\calA$ is rank-1 if $\calA=B \circ C \circ D$ for
      some vectors $B$, $C$, $D$, where $\circ$ denotes an outer product).
      \begin{itemize}
        \item Considered for signal extraction in [Kouchaki, Sanei (2013)]
        \item Requires to know number of components in advance
        \item Does not provide any form of orthogonality of components
        \item No connection between signal rank and number of components
      \end{itemize}
      \medskip
...
\begin{frame}{Some tensor decompositions}
  \begin{itemize}
    \item \bluetext{$(L_r, L_r, 1)$}: sum of tensors with elements of
      form $a_{ilk}^{(r)} = b_{il}^{(r)} d_k^{(r)}$, for some vector
      $D^{(r)}$ and matrix $\bfB^{(r)}$ of rank $L_r$ ($r$ is a sum index)
      \begin{itemize}
        \item Considered for signal extraction and decomposition in
          [De~Lathauwer~(2011)]
        \item Sparse theory and no open-source implementation
      \end{itemize}
      \medskip
    \item \bluetext{HOSVD}: $\calX = \sum_{i=1}^{I}\,
      \sum_{l=1}^{L}\, \sum_{k=1}^{K}\, z_{ilk} U_i^{(1)} \circ
      U_l^{(2)} \circ U_k^{(3)}$
      \begin{itemize}
        ...
        \item Has proven connection between number of components and signal rank
      \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}{Dtack Modifications}
  ...
  mix into one in presence of noise. Solution: using Dstack mapping.
  \medskip
...
\begin{frame}{Results and Fututre Work}
  ...
  \begin{itemize}
    \item Trying other tensor decompositions
