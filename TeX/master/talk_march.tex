...
\section{Известные результаты. Свойства тензорных алгоритмов}
...
\begin{frame}[noframenumbering]
  \centering \Large Полученные результаты
\end{frame}
...
\begin{frame}{Трудоёмкости алгоритмов}
  ...
  \begin{itemize}
    \item \bluetext{SSA}/\bluetext{MSSA}:\\
      $O(R N^* \log (N^*) + R^2 N^*)$ \medskip
    \item \bluetext{T-SSA} с использованием HOSVD:\\
      $O((R_1 + R_2 + R_3)N^*\log(N^*) + (R_1^2 + R_2^2 + R_3^2)N^*)$ \medskip
    \item \bluetext{T-MSSA} с использованием HOSVD:\\
      $O((R_1 + R_2)N^*\log(N^*) + (R_1^2 + R_2^2 + R_3)N^* + R_3^2(M + N))$
  \end{itemize}
\end{frame}

\begin{frame}{Собственные числа в тензорных методах}
  ...
  \begin{itemize}
    \item Одноканальный ряд $x_n = C$, $1 \leqslant n \leqslant N$:\\
      ...
      \] \smallskip
    \item Многоканальный ряд $x_n^{(m)} = C_m$, $1 \leqslant n \leqslant N$:\\
      \[
...
\begin{frame}{Собственные числа. Связь с матрицами}
