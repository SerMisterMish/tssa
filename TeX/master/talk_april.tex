...
\begin{frame}{Pseudo-HOSVD для ганкелевых тензоров}
  \begin{itemize}
    \item Пусть $U_i^{(n)}$ --- левые сингулярные векторы из SVD
      ...
      $\widetilde{U}_i^{(n)}$ --- из SVD $\widetilde{[\bfA_1]}$ (не
      взвешенного) \smallskip
    \item Пространства столбцов $[\bfA_n]$ и $\widetilde{[\bfA_n]}$
      совпадают $\Rightarrow$ базисы $U_i^{(n)}$ и
      $\widetilde{U}_i^{(n)}$ образуют одинаковые пространства и
      ...
      \[
        ...
        \widetilde{\bfU}_3,
      \]
      которое является разложением Таккера с ядерным тензором
      \[
    ...
    \item Полученное разложение \textbf{не является} HOSVD
  \end{itemize}
\end{frame}
...
\begin{frame}{T-SSA, T-MSSA и T-ESPRIT с использованием HOSVD}
  ...
  \begin{enumerate}
    ...
    \item \bluetext{Разложение и Приближение}. Используя $(R_1,\,
      ...
      \operatorname{Trunc}(\operatorname{HOSVD}(\calX))$ или
      $\operatorname{HOOI}(\calX)$ или
      $\operatorname{Trunc}(\operatorname{Pseudo-HOSVD}(\calX))$
      (только для одноканального $\tX$)
      \vspace{0.15cm}
...
\begin{frame}{Цели и необходимые результаты}
  ...
  \begin{itemize}
    ...
    \item Сравнение SSA и T-SSA с применением Pseudo-HOSVD
    \item Условия наибольшей точности T-MSSA
    \item Определение собственных чисел ряда, собственные числа
      сигналов и шума
    \item Нахождение трудоёмкости методов
  \end{itemize}
\end{frame}
...
\begin{frame}{T-SSA для космплексных одноканальных рядов}
  ...
  \begin{table}
    ...
    \begin{tabular}{c|c||c}
      ...
      Матричный метод & 0.137 & 6.40e-4 \\\hline
      Тензорный метод & 0.143 & 6.71e-4 \\\hline
      \begin{tabular}{c}Тензорный метод \\ (Pseudo-HOSVD)
      \end{tabular} & \textbf{0.134} &
      \textbf{6.38e-4} \\\hline
    \end{tabular}
  \end{table}
\end{frame}
...
\begin{frame}{Применение к космплексным многоканальным рядам}
  ...
  \begin{table}
    ...
    \begin{tabular}{c|c||c}
      \hline
      Задача & Выделение сигнала & Оценка параметров \\\hline
      Матричный метод & 0.101 & 1.022e-04 \\\hline
      Тензорный метод & \textbf{0.070} & \textbf{1.017e-04} \\\hline
    \end{tabular}
  \end{table}
\end{frame}
...
\begin{frame}{Оценка параметров с использованием Dstack}
  ...
  \]
  $N = 1000$, $D=10$, $\omega_1 = 0.02,\, \omega_2 = 0.0205$,
  $\xi_n$ --- белый
...
\begin{frame}{Сравнение точности одноканальных методов}
  \begin{itemize}
    \item T-SSA и T-ESPRIT с использованием HOSVD существенно менее
      точны, чем SSA
      во всех рассмотренных случаях комплексных рядов\smallskip
    \item T-SSA и T-ESPRIT с использованием Pseudo-HOSVD более
      точны, чем SSA в задачах выделения сигнала и оценки параметров
