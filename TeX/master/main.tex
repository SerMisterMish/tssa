...
\begin{statement}\label{state:htmsvd-ranks-rotation}
...
Кроме того, в результате численных экспериментов в работе~\cite{Thesis2024} было
...
\begin{theorem}
  \label{theorem:tens-ssa-rank2}
  Теорема~\ref{theorem:tens-ssa-rank} также выполняется при
  применении разложения \newdecomp/.
\end{theorem}
\begin{proof}
  Справедливость теоремы для \newdecomp/ следует из её справедливости
  для HOSVD и утверждения~\ref{state:htmsvd-ranks-rotation}.
\end{proof}
...
\begin{algorithm}[!ht]
  ...
  \begin{algorithmic}[1]
    ...
    с длиной окна $L$. \label{algstep:tmssa-inj}
...
Пусть дан $M$-мерный временной ряд $\tX$ длины $N$.
Метод \tmssa/ для выделения в ряде сигнала из шума, по аналогии с
алгоритмом \tssa/, сводится к получению как можно более точного
приближения траекторного тензора тензором меньших $n$-рангов.
Описание метода приведено в алгоритме~\ref{alg:tmssa-signal}.
...
\begin{algorithm}[!ht]
  ...
  \begin{algorithmic}[1]
    ...
    алгоритма~\ref{alg:tmssa-components}.
    \State \label{algstep:tmssa-signal-reverse}
    Восстановление:
...
\subsection{Свойства \tmssa/}
В этом разделе приведены некоторые свойства метода \tmssa/ и
...
\begin{remark}
  ...
  SSA равносильны и \tesprit/ с использованием \newdecomp/ равносильны, и
  потому данный вариант алгоритма \tesprit/ не рассматривается.
\end{remark}

По аналогии с обозначениями для алгоритма~\ref{alg:esprit},
в дальнейшем \tesprit/ будет обозначать одномерный вариант
алгоритма~\ref{alg:tesprit}, а HO-M-ESPRIT "--- многомерный.

\subsection{Выбор параметра усечения третьего направления в алгоритмах
\tesprit/ и HO-M-ESPRIT}\label{subsec:tesprit-r3}
...
\begin{corollary}
  Для получения несмещённых оценок параметров методами \tesprit/
  и HO-M-ESPRIT
...
Матрица, полученная удалением повторных столбцов из $j$-ой развёртки
...
\end{enumerate}
соответственно.
Итого, временная трудоёмкость HOSVD и \newdecomp/ траекторного
тензора одномерного
ряда имеет порядок
