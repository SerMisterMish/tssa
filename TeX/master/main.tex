...
\def\newdecomp/{HUTD}
\def\newdecompfull/{Hankel Unweighted Tucker Decomposition}
...
\tssa/ и \tesprit/ с использованием оператора Dstack.
...
В разделе~\ref{sec:cmssa-comparison} методы \tssa/ и \tmssa/
...
\begin{remark}
  Все приведённые выше суждения естественным образом обобщаются для
...
\subsection{\tssa/ для разделения компонент сигнала}\label{tssa-sep}
...
\subsection{\tssa/ для выделения сигнала из ряда}\label{subsec:tssa-signal}
Алгоритм \tssa/ для выделения в ряде сигнала из шума сводится к получению
как приближения траекторного тензора тензором меньших $n$-рангов,
которые задаются в качестве параметров алгоритма.
Метод \tssa/ для выделения сигнала представлен в
алгоритме~\ref{alg:tssa-signal}.
\begin{algorithm}[!ht]
  \caption{\tssa/ для разделения компонент сигнала.}
...
\begin{algorithm}[!ht]
  \caption{\tssa/ для выделения сигнала.}
  \label{alg:tssa-signal}
  \Input $\tX$, $L,K: 1< L,K < N,\, L + K < N + 1$, где $N$ "---
  длина $\tX$, $R_1 \in \overline{1:L}$,
  $R_2 \in \overline{1:K}$, $R_3 \in \overline{1:M}$,
  $\operatorname{Dec}$ "--- разложение Такера\\
  \Output $\widehat{\tS}$ "--- оценка сигнала $\tS$.

  \begin{algorithmic}[1]
    \State Совпадает с шагом~\ref{algstep:tssa-inj}
    алгоритма~\ref{alg:tssa-components}.
    \State Совпадает с шагом~\ref{algstep:tssa-decomp}
    алгоритма~\ref{alg:tssa-components}.
    \State
...
\begin{remark}
  Направления усечения определены и для алгоритма~\ref{alg:tssa-signal}.
\end{remark}
\begin{remark}
  Алгоритмы~\ref{alg:tssa-components} и~\ref{alg:tssa-signal}
  естественным образом обобщаются на случай $D$-\linebreak{}мерного
  траекторного тензора.
\end{remark}

\subsection{Известные свойства \tssa/}
В работе~\cite{Thesis2024} был исследован
метод \tssa/ с применением разложений HOSVD и HOOI.
...
\begin{theorem}
  ...
  Тогда существует \emph{HOSVD} тензора $\mathcal{X}$, которое
  можно представить
  в виде суммы \emph{HOSVD} тензоров $\widetilde{\mathcal{X}}$ и
  $\widehat{\mathcal{X}}$
  в том и только том случае, когда взаимно ортогональны все подряды рядов
  $\widetilde{\tX}$ и $\widehat{\tX}$ длины $L,\, K,\, M=N-L-K+2$, то есть
  \begin{enumerate}
...
установлено, что точность метода \tssa/ с использованием разложений
HOSVD и HOOI значительно меньше, чем точность базового SSA в задачах
выделения сигнала, разделения компонент и оценки параметров сигнала
при применении к вещественнозначным рядам.
...
\end{theorem}
\begin{proof}
  Справделивость теоремы для \tssa/ с использованием HOSVD уже была
...
\subsection{\tmssa/ для разделения компонент сигнала}
...
\subsection{\tmssa/ для выделения сигнала}
\label{subsec:tmssa-desc-signal}
Рассмотрим $M$-мерный временной ряд $\tX$ длины $N$.
Описание метода \tmssa/ для выделения сигнала из зашумлённого ряда приведено в
алгоритме~\ref{alg:tmssa-signal}.

\begin{algorithm}[!ht]
  \caption{\tmssa/ для выделения сигнала}
  \label{alg:tmssa-signal}
  \Input $\tX = \left(\tX^{(1)}, \ldots, \tX^{(M)}\right)^{\rmT}$,
  $L: 1< L < N$, где $N$ "--- длина $\tX$, $R_1 \in \overline{1:L}$,
  $R_2 \in \overline{1:K}$, $R_3 \in \overline{1:M}$, где $K =
  N-L+1$, $\operatorname{Dec}$ "--- некоторое разложение Такера.\\
  \Output $\widehat{\tS}$ "--- оценка сигнала $\tS$.
  \begin{algorithmic}[1]
    \State Совпадает с шагом~\ref{algstep:tmssa-inj}
    алгоритма~\ref{alg:tmssa-components}.
    \State Совпадает с шагом~\ref{algstep:tmssa-decomp}
    алгоритма~\ref{alg:tmssa-components}.
    \State Восстановление:
    сечения $\mathring{\calS}_{\cdot\cdot m}$ усредняются вдоль
    побочных диагоналей $l + k = \operatorname{const}$ для
    получения оценок $\widehat{\tS}^{(m)}$.\label{algstep:tmssa-signal-reverse}
  \end{algorithmic}
\end{algorithm}

\subsection{Известные свойства \tmssa/}
В работе~\cite{Thesis2024} был исследован
метод \tmssa/. В этом разделе приведены некоторые полученные в ней свойства
метода и вспомогательные определения.
...
В результате численных экспериментов в работе~\cite{Thesis2024} было
установлено, что в случае вещественнозначных рядов метод \tmssa/
...
Также было выявлено, что преимущество метода \tmssa/ сильно зависит
от вида сигнала, и разница в точности между MSSA и \tmssa/ для
...
следствием пункта 3 утверждeния~\ref{state:traj-mssa-mat-to-tens} и
...
\begin{algorithm}[!ht]
  ...
  $ed\in \{1, 2, 3\}$ "--- номер направления сингулярных векторов,
  ...
  \begin{algorithmic}[1]
    ...
    \[
      ...
      U_{i_3}^{(3)},
    \]
    построение матрицы $\bfU = \left[U_1^{(ed)}:U_2^{(ed)}:\ldots:
    U_{R}^{(ed)}\right]$.
    \State Решение уравнения
    \[
      \bfU^{\uparrow}=\bfU_{\downarrow}\bfZ
...
\begin{theorem}\label{theorem:tmesprit-r3}
  ...
  Пусть в алгоритме~\ref{alg:tesprit} $ed \ne 3$, а $N$ и $L$ такие,
  что $\min(L, KM) > \operatorname{rank}(\tX)$
  при $ed=1$ или $\min(K, LM) > \operatorname{rank}(\tX)$ при $ed=2$.
...
\end{theorem}
\begin{proof}
  Пусть $ed = 1$ (для $ed=2$ доказательство аналогичное). Достаточно
...
\begin{theorem}\label{theorem:tesprit-r3}
  ...
  \in (-1/2, 1/2]$,
  и пусть $N$, $L$ и $K$ такие, что $\min(I_{d'}, I_{ed}
  I_{d''}) >
  \operatorname{rank}(\tX)$, где $\{d', d''\} = \{1, 2, 3\}\setminus\{ed\}$,
...
\begin{corollary}
  При использовании метода \tmesprit/ для получения несмещённых
...
\begin{proof}
  ...
  востановленном ряде каждый элемент является циклической свёрткой
  ...
  является $n$-м элементом последовательной свёрткой векторов
...
Таким образом, методы \tssa/ и \tmssa/ имеют трудоёмкости
...
\end{description}
Кроме того, для метода HOESPRIT рассматривались все значения параметра
направления оценки $ed$ из алгоритма~\ref{alg:tesprit}.
...
\begin{table}[!ht]
  ...
  \begin{tabular}{ccccc}
    \hline
    Задача & HOSSA & SSA & 3D UTSSA & 4D UTSSA \\
    \hline
    Выделение &
...
\begin{table}[!ht]
  ...
  \begin{tabular}{ccccc}
    ...
    \end{tabular} \\
    Оценка &
    \begin{tabular}{c}6.316e-04\\ $L = (57, 42), ed = 3$
    \end{tabular} &
...
\begin{table}[!ht]
  ...
  \begin{tabular}{ccccc}
    ...
    \end{tabular} \\
    Оценка &
    \begin{tabular}{c}2.274e-03\\ $L = (63, 33), ed = 1$
    \end{tabular} &
...
\begin{table}[!ht]
  ...
  \begin{tabular}{ccccc}
    ...
    \end{tabular} \\
    Оценка &
    \begin{tabular}{c}8.911e-04\\ $L = (63, 33), ed = 1$
    \end{tabular} &
...
\begin{table}[!ht]
  ...
  \begin{tabular}{ccccc}
    ...
    \end{tabular} \\
    Оценка &
    \begin{tabular}{c}1.838e-03\\ $L = (65, 30), ed = 1$
    \end{tabular} &
...
\begin{table}[!ht]
  ...
  \begin{tabular}{ccccc}
    ...
    \end{tabular} \\
    Оценка &
    \begin{tabular}{c}1.141e-03\\ $L = (67, 32), ed = 1$
    \end{tabular} &
...
\begin{table}[!ht]
  ...
  \begin{tabular}{ccccc}
    ...
    \end{tabular} \\
    Оценка &
    \begin{tabular}{c}4.382e-04\\ $L = (62, 37), ed = 2$
    \end{tabular} &
...
\begin{table}[!ht]
  ...
  \begin{tabular}{ccccc}
    ...
    \end{tabular} \\
    Оценка &
    \begin{tabular}{c}9.196e-04\\ $L = (63, 33), ed = 1$
    \end{tabular} &
...
точность метода HOSSA существенно ниже, чем точность метода SSA во
...
параметра $ed$ в методе HOESPRIT различно для разных вариантов
сигнала, но в большинстве случаев совпадает направлением наибольшего
размера.
С другой стороны, точность методов UTSSA значимо выше точности SSA как в
задаче выделения сигнала, так и в задаче разделения компонент. Также
видно, что переход к 4-мерным тензорам в методе UTSSA не даёт
...
\begin{figure}[!ht]
  ...
  \caption{Зависимость RMSE от длины окна $L$,
  ряд~\ref{enum:compcase-1d-1_1_6}, HOSSA}
...
\begin{figure}[!ht]
  ...
  \caption{Зависимость RMSE от длины окна $L$,
  ряд~\ref{enum:compcase-1d-1_1_6}, 3D UTSSA}
...
\begin{figure}[!ht]
  ...
  \caption{Зависимость RMSE от длины окна $L$,
  ряд~\ref{enum:compcase-1d-1_1_6}, 4D UTSSA}
...
\begin{figure}[!ht]
  ...
  \caption{Зависимость RMSE от длины окна $L$,
  ряд~\ref{enum:compcase-1d-1_2_4}, HOSSA}
...
\begin{figure}[!ht]
  ...
  \caption{Зависимость RMSE от длины окна $L$,
  ряд~\ref{enum:compcase-1d-1_2_4}, 3D UTSSA}
...
\begin{figure}[!ht]
  ...
  \caption{Зависимость RMSE от длины окна $L$,
  ряд~\ref{enum:compcase-1d-1_2_4}, 4D UTSSA}
...
зависит от выбора длины окна гораздо сильнее, чем SSA. В задаче выделения
сигнала точность UTSSA в зависимости от длины окна ведёт себя похожим
образом на точность выделения в SSA, то есть близка к минимуму при
большинстве значений длины окна. Графики зависимостей RMSE от длины
окна для других вариантов сигнала имеют аналогичный вид.

Исходя из рисунков, можно
сформулировать следующую рекомендацию по выбору параметров длины окна
в методе UTSSA: пусть значения длины окна $L = (L_1,\, \ldots,\,
...
В этом разделе приведены сравнения методов MSSA/M-ESPRIT и
\tmssa//\linebreak\tmesprit/ по
...
% степеней затухания,
% полученных методом \tesprit/, от размеров траекторного тензора
...
% Однако множество длин окна в алгоритме \tesprit/, при которых
...
% параметрах невелика.
% В связи с этим, использование метода \tesprit/ для
...
% алгоритма~\ref{alg:tesprit}, соответствующего направлению
% наименьшего размера
...
% в одномерном случае в разделе~\ref{subsec:mv-comparison}.
% В качестве амплитуд $a_k^{(m)}$ были взяты независимые реализации
% случайных величин из распределения $\mathrm{CN}(0,\, 1)$,
% их приблизительные значения приведены в
% выражении~\eqref{eq:complex-amplitudes}.
...
полученных методами M-ESPRIT и \tmesprit/, от значения длины окна $L$.
...
Таким образом, метод \tmesprit/ оказался точнее стандартного метода
M-ESPRIT в задаче оценки параметров многомерного ряда для любого
выбора параметров длины окна во всех случаях,
не считая двух: оценки методом \tmesprit/ параметра $\alpha_1$ оказались
...
и параметры $R_1$ и $R_2$ в алгоритме~\ref{alg:tmssa-signal} были
взяты равными 2.
3-ранг этого сигнала в терминах \tmssa/ равен 2, поэтому параметр
$R_3$ в алгоритме~\ref{alg:tmssa-signal} был взят равным 2.
...
сигнала методами MSSA и \tmssa/ от выбора длины окна $L$ для различных
параметров степеней затухания $\alpha_1$ и $\alpha_2$.

Из графиков видно, что во всех случаях метод \tmssa/ выделяет
...
\tmssa/ в задаче выделения вещественных сигналов.

\section{Выбор направления усечения в алгоритме
\tssa/}\label{sec:trunc-dim-tssa}
В этом разделе рассматривается влияние выбора направлений усечения
в методе \tssa/ на точность в задаче выделения сигнала.
...
сигнала, восстановленного методом \tssa/ при
...
\begin{table}[!ht]
  \centering
  \caption{\tssa/: RMSE оценки сигнала~\eqref{eq:tssa-sin-signal} при
...
любом выборе
длин окна.
Но этого увеличения точности недостаточно, чтобы алгоритм \tssa/ был
точнее базового SSA.
...
\end{figure}
Как и в случае вещественного сигнала, метод \tssa/ оказывается
...
\section{Зависимость точности \tmssa/ от количества каналов}
\label{sec:r3-vs-channels}
В работе~\cite{Thesis2024} при сравнении методов MSSA и \tmssa/ по
...
основным признаком, влияющим на степень различия в точности MSSA и \tmssa/ в
...
\begin{table}[!ht]
  ...
  \caption{Зависимость RMSE оценки сигнала от количества каналов и
    стандартного отклонения шума для методов MSSA (M) и \tmssa/ (T).
...
минимальные по
параметрам длины окна значения RMSE для методов MSSA и \tmssa/ для
...
\begin{equation*}
  ...
  \operatorname{RMSE}_{\text{\tmssa/}}}{\sigma},
\end{equation*}
...
Таким образом, метод \tmssa/ наиболее целесообразно применять в случае, когда
...
метода с SSA, MSSA и \tmssa/ с точки зрения точности выделения
...
сравнения с методами MSSA и \tmssa/ на малом наборе рядов.
...
менее точно,
чем базовый SSA, а метод \tmssa/ выделяет многомерные комплексные сигналы
точнее метода MSSA.
Кроме того, было показано, что в случае, когда количество каналов
больше 3-ранга ряда, преимущество \tmssa/ растёт с увеличением
количества каналов,
