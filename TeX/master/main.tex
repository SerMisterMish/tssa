...
\def\newdecompru/{Невзвешенное разложение Таккера ганкелевого тензора}
\def\newdecomprugenitive/{невзвешенным разложением Таккера ганкелевого тензора}
% \def\newdecompru/{Неизбыточное разложение Таккера ганкелевого тензора}
\def\tssa/{Tucker-SSA}
...
Если тензор $\calA\in\bbC^{L\times K \times M}$ ганкелевый, то
...
\begin{definition}[\newdecompru/]\label{def:newdecomp}~\\
  ...
  \[
    ...
    \widetilde{\bfU}_3,
  \]
  являющееся разложением Таккера тензора $\calA$.
...
представлению разложения Таккера траекторного тензора ряда
$\tX$ в виде суммы разложений Таккера траекторных тензоров рядов $\tX_g$.
...
\begin{algorithm}[!ht]
  ...
  \[
    ...
    g\ne l,\, p \in \{1, 2, 3\},
  \]
  $\operatorname{Dec}$ "--- некоторое разложение Таккера (например HOSVD,
  HOOI, \newdecomp/).\\
...
\begin{algorithm}[!ht]
  ...
  $\operatorname{Dec}$ "--- разложение Таккера\\
...
получению представления некоторого разложения Таккера траекторного тензора
наблюдаемого сигнала $\tX$ в виде суммы разложений Таккера траекторных тензоров
...
\begin{algorithm}[!ht]
  ...
  Таккера (например HOSVD или HOOI)\\
...
\begin{algorithm}[!ht]
  ...
  N-L+1$, $\operatorname{Dec}$ "--- некоторое разложение Таккера.\\
  \Output $\widehat{\tS}$ "--- оценка сигнала $\tS$.
  \begin{algorithmic}[1]
...
\begin{remark}
  ...
  ESPRIT и \tesprit/ с использованием \newdecomp/ равносильны.
\end{remark}
...
\begin{theorem}\label{theorem:tmesprit-r3}
  ...
  Пусть в алгоритме~\ref{alg:tesprit} $d \ne 3$, а $N$ и $L$ такие,
...
\begin{theorem}\label{theorem:tesprit-r3}
  ...
  \in (-1/2, 1/2]$,
  и пусть $N$, $L$ и $K$ такие, что $\min(I_{d'}, I_d
  I_{d''}) >
...
Пусть $\tX$ --- одномерный временной ряд длины $N$.
...
затем применять к полученному ряду алгоритм \tmesprit/.
...
параметров ESPRIT и \tesprit/, а также методов выделения сигнала
SSA и \tssa/.
Описания методов \tesprit/ и \tssa/ с использованием Dstack приведены
в алгоритмах~\ref{alg:dstack-tesprit} и~\ref{alg:dstack-tssa}
соответственно.
\begin{algorithm}[!ht]
  \caption{Dstack \tesprit/ для оценки параметров.}
  \label{alg:dstack-tesprit}
  \Input $\tX$, $D, L_D: 1\leqslant D \leqslant N,\, 1 < L_D < N_D
  = \lfloor N / D \rfloor$, $\operatorname{Dec}$ "--- разложение Таккера,
  ...
  \begin{algorithmic}[1]
    ...
    \State Совпадает с шагом 2 алгоритма~\ref{alg:tesprit}.
    \State Совпадает с шагом 3 алгоритма~\ref{alg:tesprit}.
    \State Совпадает с шагом 4 алгоритма~\ref{alg:tesprit}.
...
\begin{remark}\label{remark:dstack_condition}
  ...
  алгоритм~\ref{alg:dstack-tesprit}
...
\begin{algorithm}[!ht]
  \caption{Dstack \tssa/ для оценки комплекснозначного сигнала.}
  \label{alg:dstack-tssa}
  \Input $\tX$, $D, L_D: 1 \leqslant D \leqslant N,\, 1 < L_D < N_D =
  \lfloor N / D \rfloor$, $\operatorname{Dec}$ "--- разложение Таккера,
  ...
  \begin{algorithmic}[1]
    ...
    \State Совпадает с шагом~\ref{algstep:tmssa-decomp}
    алгоритма~\ref{alg:tmssa-components}.
    \State Совпадает с шагом~\ref{algstep:tmssa-signal-reverse}
    алгоритма~\ref{alg:tmssa-signal}.
...
\end{remark}
\begin{remark}
  Теорема~\ref{theorem:tesprit-r3} о выборе параметра $R_3$
...
В этом разделе приведены сравнения методов ESPRIT и \tesprit/ по
...
\paragraph{Выводы из численных сравнений}
Метод \tmesprit/ оказался точнее стандартного метода M-ESPRIT в задаче
оценки параметров
...
замечания~\ref{remark:tmssa-trunc-dims}.
...
алгоритмах~\ref{alg:ssa-signal},~\ref{alg:esprit},~\ref{alg:dstack-tesprit},
и $R_1=R_2 = 4$ в алгоритме~\ref{alg:dstack-tssa}.
...
Dstack методов наименьшую ошибку имеет Dstack \tesprit/ с выбором $R_3=4$.
...
При выборе оптимальных длин окна методы Dstack SSA и Dstack \tssa/
