...
и в задаче оценки параметров сигнала, а также модификация метода \tssa/ с
усечением лишь части направлений и модификации методов \tesprit/ и
\tssa/ с использованием оператора Dstack.
...
В разделе~\ref{sec:tssa-method-description}
представлено описание тензорной модификации метода SSA "--- \tssa/,
и приведены некоторые определения и утверждения,
используемые далее в работе.
В разделе~\ref{sec:tmssa-desc}
описывается метод \tmssa/ для выделения сигнала и разделения
компонент и некоторые его свойства.
В разделе~\ref{sec:tesprit-description} описывается
...
В разделе~\ref{sec:mv-comparison} исследуется метод
\tesprit/ с точки зрения наличия преимущества в точности
...
Метод \tssa/ выделяет одномерные комплексные сигналы существенно
менее точно,
...
и высоком уровне шума стандартный метод ESPRIT может перестать
идентифицировать все параметры сигнала, в то время как использование
Dstack \tesprit/ в этих же условиях продолжает
