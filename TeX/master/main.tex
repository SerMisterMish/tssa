...
тензорными модификациями с точки зрения вычислительной трудоёмкости по времени.
...
Самыми трудоёмкими шагами алгоритма являются разложение и восстановление.
...
в худшем случае превращается в $O(rN^2 + r^2N)$.
...
можно снизить трудоёмкость сингулярного рзаложения траекторной
матрицы до порядка $O(r (L + K) \log(L + K) + r^2 (L + K)) = O(rN
\log(N) + r^2 N)$.
Также в этой работе было показано, что трудоёмкость этапа
восстановления имеет порядок операций $O(N \log(N))$.

Таким образом, алгоритм Basic SSA имеет временную асимптотическую
трудоёмкость порядка $O(r N \log (N) + r^2 N)$.
Кроме того, в статье~\cite{mssa2} было показано, что тот же порядок трудоёмкости
...
$N^{*}$ "---
общее число элементов в изначальном временном ряде (например $N^{*} =
NP$ для $P$-канального ряда длины $N$).
...
Выкидывание таких лишних столбцов из матрицы приводит её к
...
Аналогично для матриц-развёрток по остальным направлениям.
