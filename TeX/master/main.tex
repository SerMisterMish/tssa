...
\section{HOSVD ганкелевого тензора. HTMSVD}
...
\begin{definition}[Мультимодовое сингулярное разложение ганкелевого
  тензора]\label{def:HTMSVD}~\\
  ...
  Построенное таким образом разложение будем называть мультимодовым
  сингулярным разложением ганкелевого тензора (Multimode Hankel
  Tensor SVD --- HTMSVD). Столбцы $\widetilde{U}_j^{(n)}$ матриц
  $\widetilde{\bfU}_n$ будем называть $n$-сингулярными векторами
  тензора $\calA$ в терминах HTMSVD.
\end{definition}

\begin{remark}\label{remark:htmsvd-ranks-rotation}
  Пусть $\bfU_n$ --- матрицы сингулярных векторов $n$-го направления
  тензора $\calA$, $\widetilde{\bfU}_n$ --- матрицы сингулярных
  векторов $n$-го направления в терминах HTMSVD. Пространства
  столбцов матриц $\bfU_n$ и $\widetilde{\bfU}_n$ по построению
  совпадают с пространствами столбцов матриц $[\bfA_n]$ и
  $\widetilde{[\bfA_n]}$ соответственно. В свою очередь, пространства
  столбцов $[\bfA_n]$ и $\widetilde{[\bfA_n]}$ также совпадают. Из
  этого следует, что
  \begin{enumerate}
    \item Число сингулярных векторов $\widetilde{U}_i^{(n)}$ в терминах HTMSVD,
    ...
    \item Для любого $n\in \{1, 2, 3\}$ существует унитарная
      ...
      терминах HOSVD и HTMSVD отличаются только преобразованием-поворотом.
  \end{enumerate}
\end{remark}
...
\begin{algorithm}[!ht]
  ...
  \begin{algorithmic}[1]
    ...
    Разложение: применение к $\mathcal{X}$ HOSVD, HOOI или HTMSVD с
    параметрами $n$-рангов
    $R_1, R_2, R_3$
    \begin{equation}
...
В этом разделе приведены некоторые свойства метода HO-SSA и
...
Теорема~\ref{theorem:separability} была доказана в
работе~\cite{Thesis2024}. Из неё следует, что понятие слабой
разделимости ряда из теории SSA применимо в тензорном случае при
использовании HOSVD или HOOI в качестве разложения.
\begin{corollary}
...
\begin{theorem}
  ...
  HOSVD и\linebreak HTMSVD траекторного тензора $\mathcal{X}$,
  построенного по этому ряду с длинами окна $L$ и $K$, будет равно $d$.
\end{theorem}
\begin{proof}
  Доказательство теоремы для HOSVD приведено в работе~\cite{Thesis2024}.
  Справедливость теоремы для HTMSVD следует из её справедливости для HOSVD и
  замечания~\ref{remark:htmsvd-ranks-rotation}.
\end{proof}
...
\begin{remark}
  Так как сингулярные векторы $d$-го направления траекторного тензора в терминах
  HTMSVD по определению совпадают с левыми сингулярными векторами
  траекторной матрицы ряда с длиной окна $I_d$, то базовый алгоритм
  SSA равносильны и HO-ESPRIT с использованием HTMSVD равносильны, и
  потому данный вариант алгоритма HO-ESPRIT не рассматривается.
\end{remark}
...
получения сингулярных векторов в терминах HTMSVD достаточно вычислить
SVD полученной матрицы без использования весов.

Таким образом, вычисления первых $r_1$, $r_2$ и $r_3$ компонент HOSVD и HTMSVD
...
\end{enumerate}
соответственно.
Итого, временная трудоёмкость HOSVD и HTMSVD траекторного тензора одномерного
ряда имеет порядок
