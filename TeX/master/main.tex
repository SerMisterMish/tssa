...
\group{%
  ...
  Основная образовательная программа ВМ.5751.2024 <<Математическое
  моделирование, программирование и искусственный интеллект>>
}
\date{\number\year}
\institution{
...
\topic{Tensor modifications of singular spectrum analysis using
...
Различные подходы к применению тензоров для анализа временных рядов
рассматривались в
работах~\cite{hosvd-hooi-separation,Papy2009,TSSA,TSSA-improved,DeLathauwer2011,TrungLe2024}.
В основном, подходы различаются рассматриваемой задачей, например
задача выделения сигнала или оценки параметров сигнала, и
используемым тензорным разложением.
В частности, в работах~\cite{hosvd-hooi-separation,Papy2009} использовалось
сингулярное разложение высшего порядка (HOSVD)~\cite{hosvd} в задаче
оценки параметров.
Задача выделения сигнала рассматривалась в
работах~\cite{TSSA,TSSA-improved}, где использовалось
каноническое разложение (CPD)~\cite{parafac1, parafac2},
в работе~\cite{DeLathauwer2011}, где использовалось $(L_r,\, L_r,\,
1)$-разложение, и в работе~\cite{TrungLe2024}, где использовалось
разложение T-SVD~\cite{Kilmer2011,Kilmer2013}.
Разложения HOSVD и T-SVD являются частными случаями разложений
Такера~\cite{tucker} и в некотором смысле расширяют SVD. Кроме того,
среди указанных разложений HOSVD имеет наибольшее число свойств,
аналогичных свойствам SVD.
...
\tesprit/ в задаче оценки параметров сигнала, а также зависимость точности
метода \tmssa/ от увеличения числа каналов.
...
Все определения и утверждения из этого раздела можно найти в
работах~\cite{hosvd, Kilmer2011, Kilmer2013, tensor-bg, tensor-bg2,
tensor-bg3}, за исключением определения~\ref{def:all-unitarity} и
свойства~\ref{property:tucker-basis-change}, для которого
приведено доказательство. Термины на русском языке взяты из
работы~\cite{tensor-rus}.
...
\begin{property}
  [Связь $n$-ранга тензора и ранга его развёртки по измерению
  $n$]\label{property:n-rank-unfold}
...
\begin{definition}\label{def:all-unitarity}
  Разложение Такера, в котором все матрицы $\bfU^{(k)}$ являются
  унитарными, в данной работе будем называть удовлетворяющим условию
  полной унитарности или полноунитарным.
\end{definition}

\begin{property}\label{property:tucker-basis-change}
  Пусть разложение Такера тензора $\calA \in \bbC^{I_1 \times I_2
  \times \ldots \times I_M}$ имеет вид~\eqref{eq:tucker-decomp}, и
  удовлетворяет условию полной унитарности. Определим
  $\widetilde{\bfU}^{(k)} = \bfQ_k\bfU^{(k)}$, где $\bfQ_k\in
  \bbC^{J_k\times J_k}$ "--- тоже унитарные матрицы, и
  \[
  ...
  \]
  также является полноунитарным разложением Такера тензора $\calA$.
\end{property}
\begin{proof}
...
\begin{theorem}
  ...
  в котором
  \begin{enumerate}
    \item Выполняется условие полной унитарности:
      $\mathbf{U}^{(n)}=\left[U_1^{(n)}:\, U_2^{(n)}:\ldots:\,
...
\begin{statement}
  ...
  $M$ матриц $\mathbf{U}^{(n)}$, составленных из левых сингулярных
  векторов соответствующих развёрток. Определим тензор $\calZ$
  следующим образом
  \[
  ...
  \]
  Тогда исходный тензор $\calA$ можно представить в
  виде разложения~\eqref{eq:hosvd}, и это разложение является
  HOSVD тензора $\calA$.
...
\begin{property}[Норма]
  ...
  \begin{equation}\label{eq:hosvd-norm}
...
\begin{remark}
  Равенство $\|\mathcal{A}\|^2=\|\mathcal{Z}\|^2$ справедливо для
  любого полноунитарного разложения Такера, однако множества
  суммирования в равенстве~\eqref{eq:hosvd-norm} могут отличаться.
\end{remark}
...
\begin{equation}
  ...
  \widetilde{[\bfA_1]} = \left(
    \begin{array}{ccccc|c|c|c}
      ...
      \vdots & \vdots & \vdots & \ddots & \vdots & \vdots & x_{K +
      4} & \ddots\\
      x_L & x_{L + 1} & x_{L + 2} & \ldots & x_{K + L + 1} &
...
\begin{remark}
  По построению, HUTD является полноунитарным разложением.
\end{remark}
...
\begin{statement}\label{state:newdecomp-ranks-rotation}
  ...
  \begin{enumerate}
    ...
    \item Для любого $n\in \{1, 2, 3\}$ существует унитарная
      ...
      терминах HOSVD и \newdecomp/ отличаются только
      преобразованием-поворотом.
  \end{enumerate}
\end{statement}
\begin{proof}
  Обозначим $\operatorname{span}(\bfX)$ "--- линейная оболочка
  столбцов матрицы
...
\begin{definition}[Траекторный тензор ряда]
  ...
  Траекторным тензором ряда $\tX$ с длиной окна $\bfL\in
  \bbN^{D-1}$ такой, что
...
\begin{definition}
  ...
  разделимыми в терминах \tssa/ с разложением
  $\operatorname{Dec}$, если существует разложение $\operatorname{Dec}$ тензора
  $\mathcal{X}$, которое можно представить
  в виде суммы разложений $\operatorname{Dec}$ тензоров
  $\widetilde{\mathcal{X}}$ и $\widehat{\mathcal{X}}$.
\end{definition}
...
компонент и оценки параметров сигнала при применении к
вещественнозначным рядам.
...
\begin{theorem}
  ...
  \begin{equation}
    d\leqslant\min(L, K, M), \quad M = N - L - K + 2,
    \label{eq:tensor-admissability}
  \end{equation}
  количество ненулевых сингулярных чисел по каждому направлению в
  HOSVD и HUTD траекторного тензора $\mathcal{X}$,
  построенного по этому ряду с длинами окна $L$ и $K$, будет равно $d$.
\end{theorem}
\begin{proof}
  Временной ряд $\tX$ имеет SSA-ранг $d$, если для любой длины окна
  $L'$ такой, что $d \leqslant \min(L', N-L'+1)$, ранг траекторной
  матрицы $\bfX$ этого ряда, построенной по длине
  окна $L'$, равен $d$. Это равносильно тому, что для любых таких
  $L'$ размерность линейного пространства, построенного на столбцах
  $\bfX$, равна $d$. Столбцы матрицы $\bfX$ имеют вид $\left(x_i,
  x_{i+1}, \ldots, x_{i + L' - 1}\right)^{\rmT}$, и при подстановке
  $L$, $K$ или $M$ вместо $L'$,такой же вид имеют векторы
  траекторного тензора $\calX$ по соответствующим направлениям. Таким
  образом, если выполняется
  неравенство~\eqref{eq:tensor-admissability}, то все $n$-ранги тензора
  $\calX$ равны $d$. Из свойства~\ref{property:n-rank} следует
  справедливость теоремы для HOSVD. Справедливость теоремы для
  \newdecomp/ следует из её справедливости для HOSVD и
  утверждения~\ref{state:newdecomp-ranks-rotation}.
\end{proof}
\begin{remark}
...
\begin{theorem}
  \label{theorem:ut-ssa-no-separability}
  Пусть $\widetilde{\tX}=(\tilde{x}_1,\ldots , \tilde{x}_N)$ и
  $\widehat{\tX}=(\hat{x}_1,\ldots , \hat{x}_N)$ "--- временные
  ряды длины $N$.
  Тогда $\widetilde{\tX}$ и $\widehat{\tX}$ не разделимы в терминах
  \newtssa/ при использовании трёхмерного траекторного тензора и при
  любом выборе длины окна $(L_1, L_2)$.
\end{theorem}
\begin{proof}
  Выберем некоторую длину окна $(L_1, L_2)$, и пусть траекторные
  тензоры рядов $\widetilde{\tX}$, $\widehat{\tX}$, $\tX =
  \widetilde{\tX} + \widehat{\tX}$ равны соответственно
  $\widetilde{\mathcal{X}},\, \widehat{\mathcal{X}},\, \mathcal{X}$.
  Размер третьего направления этих тензоров равен $L_3 = N - L_1 - L_2 + 2$.
  Также определим $K_n = N - L_n + 1$.
  Если ряды $\widetilde{\tX}$ и $\widehat{\tX}$ разделимы в терминах
  \newtssa/ с длиной окна $(L_1, L_2)$, то существует такое \newdecomp/
  тензора $\mathcal{X}$, что для любого $n$ матрицу $\bfU^{(n)}$
  сингулярных векторов в терминах \newdecomp/ этого
  тензора, соответствующих ненулевым сингулярным числам,
  можно однозначно разделить по столбцам на две матрицы
  $\widetilde{\bfU}^{(n)}$ и $\widehat{\bfU}^{(n)}$, которые являются
  матрицами \newdecomp/-сингулярных векторов тензоров
  $\widetilde{\calX}$ и $\widehat{\calX}$ соответственно.
  Из теории SSA известно, что это возможно в том и только том случае,
  когда ортогональны все подряды рядов $\widetilde{\tX}$ и
  $\widehat{\tX}$ с длинами, равными $L_n$ и $K_n$.
  В частности, должны быть ортагональны подряды с длинами $L_1$,
  $L_2$ и $K_3$, а так как $K_3 = N - L_3 + 1 = L_1 + L_2 - 1$, то
  это возможно только
  в том случае, если один из рядов $\widetilde{\tX}$ или
  $\widehat{\tX}$ состоит только из нулевых элементов.
\end{proof}

\begin{remark}\label{remark:approx-utssa-separability}
  В теории SSA также существует понятие приближённой разделимости,
  основанное на асимптотических свойствах метода. Это свойство
  позволяет обосновывать применение SSA в задаче разделения
  компонент, даже если не выполняются условия разделимости.
  Несмотря на невозможность выполнения условий разделимости методом
  \newtssa/, его применение в задаче разделения компонент может в
  дальнейшем быть обосновано аналогичными асимптотическими свойствами.
\end{remark}
...
\begin{algorithm}[!ht]
  ...
  соответственно.
  \begin{algorithmic}[1]
    \State Вложение: построение траекторного тензора $\calX =
...
\begin{proof}
  ...
  матрице $\tX^{(m)}$, то есть $d_i$ раз, где
...
\section{Оптимизация временной трудоёмкости тензорных
методов}\label{sec:efficiency}
...
\begin{proof}
  ...
  \begin{equation}\label{eq:fft-conv-rec}
    \left(u^{(1)}*u^{(2)}*u^{(3)}\right) = IFFT_N\left(FFT(u^{(1)})
    \odot FFT(u^{(2)}) \odot FFT(u^{(3)})\right).
  \end{equation}
\end{proof}

Полный алгоритм оптимизированного метода \tssa/ для восстановление
одноранговой компоненты представлен в алгоритме~\ref{alg:tssa-optimized}.
\begin{algorithm}[!ht]
  \caption{Оптимизированный алгоритм \tssa/}
  \label{alg:tssa-optimized}
  \Input $\tX$, $L,\,K: 1< L,\,K < N,\, L + K < N + 1$, где $N$ "---
  длина $\tX$, $R_1,\, R_2,\, R_3$, $i_1,\, i_2,\, i_3:\: 1 \leqslant
  i_j \leqslant R_j$, $\operatorname{Dec}$ "--- некоторое разложение
  Такера (например HOSVD, \newdecomp/).\\
  \Output $\widehat{\tX}_{i_1i_2i_3}$ "--- оценка компоненты
  разложения, соответствующей индексам $i_1$, $i_2$, $i_3$.
  \begin{algorithmic}[1]
    \State \label{algstep:tssa-opt-inj}
    Вложение: построение траекторного тензора $\mathcal{X}
    =\calT_{\textrm{T-SSA}}^{(L, K)} (\tX)$.
    \State \label{algstep:tssa-opt-decomp}
    Разложение: для каждого $n \in \overline{1:3}$
    \begin{enumerate}
      \item нахождение $n$-развёртки $\left[\bfX\right]_n$,
      \item удаление из $\left[\bfX\right]_n$ избыточных столбцов,
      \item если $\operatorname{Dec} = \operatorname{HOSVD}$, умножение на
        весовую матрицу $\bfD$,
      \item применение к полученной матрице быстрого SVD для
        нахождения матрицы $R_n$ левых сингулярных векторов $\bfU^{(n)}$,
    \end{enumerate}
    \State \label{algstep:tssa-opt-core}  Вычисление ядра разложения \[
      \calZ = \calX \times_1
      \left(\bfU^{(1)}\right)^{\rmH} \times_2
      \left(\bfU^{(2)}\right)^{\rmH}\times_3 \left(\bfU^{(3)}\right)^{\rmH}.
    \]
    \State Восстановление: по компонентам $z = \calZ_{i_1i_2i_3},\,
    u^{(j)} = U^{(j)}_{i_j}$, используя
    уравнения~\eqref{eq:recon_conv_sum} и~\eqref{eq:fft-conv-rec},
    восстановление элементов ряда $\widehat{\tX}_{i_1i_2i_3}$.
  \end{algorithmic}
\end{algorithm}
...
\begin{proof}
  ...
  умноженной на диагональную матрицу весов $\bfD =
  \operatorname{diag}(\sqrt{d_1},\, \sqrt{d_2},\, \ldots,\,
  \sqrt{d_N})$, где $d_i$ определены в
...
\begin{figure}[!ht]
  \centering
  \includegraphics[width=\linewidth]{compcase-1d-all-1_2_4.pdf}
  \caption{Зависимость RMSE от длины окна $L$,
  ряд~\ref{enum:compcase-1d-1_2_4}, 4D \newtssa/}
  \label{fig:compcase-1d-all-1_2_4}
\end{figure}
На рисунке~\ref{fig:compcase-1d-all-1_2_4} представлены графики
зависимости RMSE методов HO-SSA и UT-SSA от длин окна для
ряда~\ref{enum:compcase-1d-1_2_4} в задаче выделения сигнала. Кроме
того, для удобства сравнения проведена пунктирная линия на уровне
минимального RMSE, полученного для данного ряда методом SSA.
Графики для остальных рассмотренных рядов имеют аналогичный вид.

Исходя из рисунков, можно
сформулировать следующую рекомендацию по выбору параметров длины окна
в методе 3D \newtssa/: пусть значения длины окна $L = (L_1, L_2,
L_3)$ упорядочены в порядке убывания, тогда рекомендуется
выбирать $L_1 \in [N / 3,\, 2 N / 3]$ и $L_2 \in [L_1 / 3,\, 2
L_1 / 3]$.
...
задачах выделения и разделения компонент сигнала, но только в случае
трёхмерного траекторного тензора. Переход к четырёхмерным тензорам в
алгоритме \tssa/ не привёл к увеличению точности метода.

Кроме того, численно было показано, что метод HO-MSSA имеет численное
преимущество в задачах выделения и разделения компонент
