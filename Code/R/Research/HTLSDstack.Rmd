...
for (sd_k in sds[sds != 0]) {
  ext_rmse_oth <- filter(extreme_rmse, sd == sd_k, Method != "esprit")
  ext_rmse_esprit <- filter(extreme_rmse, sd == sd_k, Method == "esprit")
  p1 <- rmse_df |> filter(sd == sd_k) |>
    ...
    annotate(
      "label",
      x = (Ls_esprit[Ls_len] + Ls_esprit[1]) * 0.38,
      y = min(ext_rmse_oth$min) * (1.01 + (sd_k == 0.6) * 4),
  ...
  p2 <- rmse_df |> filter(sd == sd_k) |>
    ...
    annotate(
      "label",
      x = (Ls_htlsd[Ls_len] + Ls_htlsd[1]) * 0.38,
      y = min(ext_rmse_esprit$min) * 1.015,
...
for (sd_k in sds[sds != 0]) {
  ext_rmse_oth <- filter(extreme_rmse, sd == sd_k, Method != "ssa")
  ext_rmse_esprit <- filter(extreme_rmse, sd == sd_k, Method == "ssa")
  p1 <- rmse_df |> filter(sd == sd_k) |>
    ...
    annotate(
      "label",
      x = (Ls_ssa[Ls_len] + Ls_ssa[1]) * 0.38,
      y = min(ext_rmse_oth$min) * 0.95,
      label = paste0("Minimum for other methods\n(achieved on ", methods_names[[ext_rmse_oth$Method[which.min(ext_rmse_oth$min)]]], ")")
    ) +
  ...
  p2 <- rmse_df |> filter(sd == sd_k) |>
    ...
    annotate(
      "label",
      x = (Ls_dstack[Ls_len] + Ls_dstack[1]) * 0.38,
      y = min(ext_rmse_esprit$min) * 1.01,
      label = paste0("Minimum for SSA")
    ) +
    scale_linetype_discrete(labels = c("SSADstack", "Tensor, r3 = 4")) +
    scale_color_discrete(labels = c("SSADstack", "Tensor, r3 = 4")) +
    ggtitle(paste0("Dstack methods, sd = ", sd_k)) +
    theme_minimal() +
    xlab("L")
