...
ssa_ev <- ssa(noise, L, kind = "mssa", neig = min(L, K * P))$sigma
...
invisible(lapply(seq_along(ev), \(i) {
  cat("  Dim", i, "-", ev[[i]], '\n')
  cat(
    "  Marchenko–Pastur Bounds for eigenvalues:",
...
## Relation of 3-mode eigenvalues and ts matrix eigenvalues 
```{r}
sm_ev <- svd(noise)$d

norm_factor <- noise_tens_fnorm / fnorm_complex(noise)
ind <- 1:min(P, N)
sm_ev <- sm_ev[ind]; ev[[3]] <- ev[[3]][ind]

cat("TEV ~ MEV correlation:", cor(ev[[3]], sm_ev), '\n')
cat("TEV / (MEV * factor):\n ", ev[[3]] / sm_ev / norm_factor, '\n')
cat("TEV upper bounds / TEV:\n ", sm_ev * sqrt(min(L, K)) / ev[[3]], '\n')
```
