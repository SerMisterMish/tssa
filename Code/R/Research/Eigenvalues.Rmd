...
noise_tens_fnorm <- fnorm_complex(tens3(noise, L, "mssa"))
sd_est <- sqrt(mean(noise^2))
sd_tens_est <- sqrt(mean(noise_tens@data^2))
ev <- noise |> tens_mssa_decompose(L, svd.method = "primme", neig=484, status = FALSE) |>
      get_tensor_eigenvalues()
ev_sums <- ev |> sapply(\(x) sum(x^2)) |> sqrt()

ssa_ev <- ssa(noise, L, kind = "mssa", neig=484)$sigma
ssa_ev_sum <- sqrt(sum(ssa_ev^2))

cat("Tensor eigenvalues:\n")
invisible(lapply(seq_along(ev), \(i) cat("  Dim", i, "-", ev[[i]][1:min(5, length(ev[[i]]))], '\n')))
cat("Tensor eigenvalue sums:", ev_sums, '\n')
cat("Matrix eigenvalues:\n ", ssa_ev[1:min(5, length(ssa_ev))], '\n')
