...
# In this parameters matrices columns correspond to summnands and rows to channels
...
# The results is either a 1d ts, or a matrix, where each column is a 1d ts -- channel.
...
# rates_case <- "_dsr"
# different by time series rates
# rates.true <- matrix(rep(c(-0.01, -0.02), P), nrow = P)
# rates_case <- "_dtr"
...
# freqs_case <- "_dsf"
# different by time series freqs
# freqs.true <- matrix(rep(c(0.2, 0.22), P), nrow = P)
# freqs_case <- "_dtf"
...
# phases_case <- "_ep"
phases.true <- matrix(rep(0, P * 4), nrow = P)
phases_case <- "_ep"
# different by time series phases
# phases.true <- matrix(c(0, pi / 3, 0, pi / 3), nrow = P)
# phases_case <- "_dtp"
...
```{r}
full_case <- paste0(n_case, P_case, rates_case, freqs_case, A_case, phases_case, complex_case)
...
if (is.matrix(signal)) {
  ...
  save(ssa_err, tssa_err, file = paste0("./Research/comp_results/cases_errors/four_periodics_mv", full_case, ".RData"))
} else {
