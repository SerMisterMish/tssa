...
max.r <- 6
tmssa_method <- "tsvd"
params_grid_hossa$L_homssa <- c(3, seq(from = 10, to = N %/% 2, by = 10))
tsvd_tssa_err <- calc_errors_by_params(params_grid_hossa, R, pred_homssa_rec, rmse_ts_nd, signal, seed = 5)
  save(tsvd_tssa_err, file = paste0("./Research/comp_results/cases_errors/", S_case, "_mv", full_case, "_tsvd.RData"))
```
...
```{r}
print_errors_m(tsvd_tssa_err)
(tsvd_tssa_err_summ <- summarise_errors(rename(tsvd_tssa_err, L = L_homssa)))
```
...
```{r}
P <- 20
load_name <- sprintf("Research/comp_results/cases_errors/one_periodic_mv_c%d_nr_ef_da_ep_r.RData", P)
load(load_name)
summarise_errors(rename(ssa_err, L = L_mssa)) |> select(sd, best, best_L) |> round(3)
summarise_errors(rename(tssa_err, L = L_homssa)) |> select(sd, best, best_L) |> round(3)
```

```{r}
P <- 40
load_name <- sprintf("Research/comp_results/cases_errors/one_periodic_mv_c%d_Ler_ef_dap1_ep_r.RData", P)
load(load_name)
summarise_errors(rename(ssa_err, L = L_mssa)) |> select(sd, best, best_L) |> round(3)
summarise_errors(rename(tssa_err, L = L_homssa)) |> select(sd, best, best_L) |> round(3)
```
...
```{r}
P <- 40
load_name <- sprintf("Research/comp_results/cases_errors/two_periodics_mv_long_c%d_dsr_dsf_dap1_ep_r.RData", P)
load(load_name)
summarise_errors(rename(ssa_err, L = L_mssa)) |> select(sd, best, best_L) |> round(3)
summarise_errors(rename(tssa_err, L = L_homssa)) |> select(sd, best, best_L) |> round(3)
```
