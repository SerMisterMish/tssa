...
s <- map(1:series.number, function(x) runif(1, 1, 1) * sin(1:N * pi / 3))
...
mssa.errors <- function(Ls) {
  ...
  for (l in seq_along(Ls)) {
    ...
    s.tens <- fold(mat.f, 1, 2:3, modes = c(L, N - L + 1, 2))
    capture.output({ h <- rTensor::hosvd(s.tens) })
    tens <- ttl(h$Z[1:r, 1:r, 1:min(r, 2), drop = FALSE], list(h$U[[1]][, 1:r, drop = FALSE], h$U[[2]][, 1:r, drop = FALSE], h$U[[3]][, 1:min(r, 2), drop = FALSE]), 1:3)
