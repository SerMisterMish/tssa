...
s <- map(1:series.number, function(x) runif(1, 1, 4) * sin(1:N * pi / 3))
...
u.svd <- svd(mat)
# signal subspace
true.subspace <- u.svd$u[, 1:r, drop = FALSE]
```
...
for (i in 1:iterations) {
  ...
  u.n.svd <- svd(mat.n)
  est.subspace <- u.n.svd$u[, 1:r, drop = FALSE]
  ...
  tens <- ttl(h$Z[1:r, 1:r, 1, drop = FALSE], list(h$U[[1]][, 1:r, drop = FALSE], h$U[[2]][, 1:r, drop = FALSE], h$U[[3]][, 1, drop = FALSE]), 1:3)
...
# signal3 <- 40 * cos(2 * pi * 1:N / 12 + pi / 20)
mres <- list()
for (series.number in series.numbers) {
  ...
  # signal <- cbind(signal1, signal2, signal3)
  R <- 100
  ...
  mssa.errors <- function(Ls) {
    ...
    for (l in seq_along(Ls)) {
      ...
      capture.output({ h <- rTensor::hosvd(s.tens) })
      tens <- ttl(h$Z[1:r, 1:r, 1:series.number, drop = FALSE], list(h$U[[1]][, 1:r, drop = FALSE], h$U[[2]][, 1:r, drop = FALSE], h$U[[3]][, 1:series.number, drop = FALSE]), 1:3)
...
for (i in series.numbers) {
  print(i)
  print(rowMeans(simplify2array(mres[[toString(i)]][1,])))
  print(rowMeans(simplify2array(mres[[toString(i)]][2,])))
}
```


```{r}
rowMeans(simplify2array(mres[1,]))
rowMeans(simplify2array(mres[2,]))
```
